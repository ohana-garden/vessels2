...
class CodeLoader:
    ...
    # =========================================================================
    # Bootstrap - Load from filesystem into DB
    # =========================================================================

    async def bootstrap_from_filesystem(self, directories: Optional[list[str]] = None) -> int:
        """
        Bootstrap: Load all code from filesystem into FalkorDB.

        This is run once to migrate from file-based to DB-based code.

        Args:
            directories: Directories to load from (default: standard A0 dirs)
        ...
        type_mappings = {
            "/tools/": CodeType.TOOL,
            "/extensions/": CodeType.EXTENSION,
            "/helpers/": CodeType.HELPER,
            "/api/": CodeType.API,
            "/instruments/": CodeType.INSTRUMENT,
        }
        ...
        for directory in directories:
            ...
            for file_path in glob_module.glob(pattern, recursive=True):
                ...
                try:
                    ...
                    for pattern_str, ct in type_mappings.items():
                        if pattern_str in rel_path:
                            code_type = ct
                            break

                    await self.store(rel_path, content, code_type)
                    loaded += 1

                except Exception as e:
                    PrintStyle.error(f"Failed to load {file_path}: {e}")

        PrintStyle.standard(f"Bootstrap complete: {loaded} files loaded into FalkorDB")
        return loaded
