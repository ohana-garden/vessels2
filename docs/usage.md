# Usage Guide
This guide explores usage and configuration scenarios for Vessels. You can consider this as a reference post-installation guide.

![Utility Message with Solutions](res/memory-man.png)

## Basic Operations
Vessels provides several basic operations through its interface:

### Restart Framework
The Restart button allows you to quickly restart the Vessels framework without using the terminal:
...
### Action Buttons
Located beneath the chat input box, Vessels provides a set of action buttons for enhanced control and visibility:
...
### File Browser: Manage files in the Vessels environment
...
### File Attachments
Vessels supports direct file attachments in the chat interface for seamless file operations:
...
## Tool Usage
Vessels's power comes from its ability to use [tools](architecture.md#tools). Here's how to leverage them effectively:

- **Understand Tools:** Vessels includes default tools like knowledge (powered by SearXNG), code execution, and communication. Understand the capabilities of these tools and how to invoke them.

## Example of Tools Usage: Web Search and Code Execution
Let's say you want Vessels to perform some financial analysis tasks. Here's a possible prompt:
...
> [!NOTE]
> The first run of `code_execution_tool` may take a while as it downloads and builds the Vessels Docker image. Subsequent runs will be faster.

This example demonstrates how to combine multiple tools to achieve an analysis task. By mastering prompt engineering and tool usage, you can unlock the full potential of Vessels to solve complex problems.

## Multi-Agent Cooperation
One of Vessels's unique features is multi-agent cooperation.
...
* **Hierarchy:** Vessels uses a [hierarchical structure](architecture.md#agent-hierarchy-and-communication), with superior agents delegating tasks to subordinates.  This allows for structured problem-solving and efficient resource allocation.

![](res/physics.png)
![](res/physics-2.png)

## Prompt Engineering
Effective prompt engineering is crucial for getting the most out of Vessels. Here are some tips and techniques:

* **Be Clear and Specific:** Clearly state your desired outcome.  The more specific you are, the better Vessels can understand and fulfill your request.  Avoid vague or ambiguous language.
...
## Voice Interface
Vessels provides both Text-to-Speech (TTS) and Speech-to-Text (STT) capabilities for natural voice interaction:
...
### File Browser
Vessels provides a powerful file browser interface for managing your workspace:
...
> [!NOTE]
> The files browser allows the user to go in the Vessels root folder if you click the `Up` button, but the working directory of Agents will always be `/work_dir`
>
- **File Operations**:
...
> [!TIP]
> The File Browser integrates seamlessly with Vessels's capabilities. You can reference files directly in your conversations, and the agent can help you manage, modify, and organize your files.

## Backup & Restore
Vessels provides a comprehensive backup and restore system to protect your data and configurations. This feature helps you safeguard your work and migrate Vessels setups between different systems.
...
#### What Gets Backed Up
By default, Vessels backs up your most important data:
...
The restore process allows you to recover your Vessels setup from a previous backup:
...
* **Before System Updates**: Backup before updating Vessels or system components
...
* **External Storage**: Keep backup files in a safe location outside Vessels
...
#### System Migration
Moving Vessels to a new server or computer:
1. Create a complete backup on the original system
2. Install Vessels on the new system
...
#### Team Collaboration
Sharing Vessels configurations:
...
> [!TIP]
> The backup system is designed to work across different operating systems and Vessels installations. Your backups from a Windows system will work on Linux, and vice versa.
